...
void cpu_exclusive_scan(const std::vector<int> & input, std::vector<int> & rOutput)
{
  int acc_ = 0;
  rOutput[0] = acc_;
...
int main(void)
{
  ...
  cpu_exclusive_scan(h_input_, h_output_);
  ...
  gpu_scan_blellochv2<<<bpg_, tpb_, kNumBytes>>>(d_input_, d_output_, kNumElements);
